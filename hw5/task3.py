...
def everything_for_your_cat(cats_data):
    ...
    cat_owners = {}
    for i in cats_data:
        name, age, name_own, surname_own = i
        buffer = f'{name_own} {surname_own}'
        if buffer in cat_owners:
            cat_owners[buffer].append((name, age))
        else:
            cat_owners[buffer] = [(name, age)]
    our_str = ''
    for person, cat in cat_owners.items():
        our_str += person + ': '
        cat_info = [cat_name + ', ' + str(cat_age) for cat_name, cat_age in cat]
        our_str += '; '.join(cat_info) + '\n'
    return our_str
